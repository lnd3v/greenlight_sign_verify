...
impl Greenlight {
    pub fn new(secret: Vec<u8>) -> Greenlight {
        Greenlight {
    ...
    pub async fn run(&mut self) {
        ...
        let mut node: gl_client::node::ClnClient = scheduler.schedule(tls).await.expect("asdf");
        let info = node
...
#[tokio::main]
async fn main() {
    let (phrase, _seed, secret) = Greenlight::generate_seed();
    let mut greenlight = Greenlight::new(secret);
    println!("{}", phrase);
    greenlight.run().await;
}
